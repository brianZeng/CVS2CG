...
function Color(rgb,a){
    ...
    if(rgb instanceof Array){
        this.alpha=a==undefined?1:parseFloat(a);
        this.rgb=rgb.slice(0,3).map(function(num){
            return +num/255
        });
    }
}
...
function parseColor(color,a){
    ...
    if(color instanceof Array)return new Color(color,a);
        ...
        if(match=color.match(/rgb\((\d+(\.\d+)?),(\d+(\.\d+)?),(\d+(\.\d+)?)\)/i)){
            ...
            throw Error('not support rgba')
        }
        else return key2Color(color);
    }
}
